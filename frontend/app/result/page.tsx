...
export default function ResultPage() {
  ...
  if (!profile) {
    return <LoadingSpinner text="사주 데이터를 불러오는 중..." />;
  }
      ...
      <div className="container">
          ...
          <div style={{
            ...
            <span className={`element-badge ${getElementClass(analysis.dayMasterElement)}`}>
              {analysis.dayMasterElement}({
                analysis.dayMasterElement === '목' ? '木' :
                analysis.dayMasterElement === '화' ? '火' :
                analysis.dayMasterElement === '토' ? '土' :
                analysis.dayMasterElement === '금' ? '金' : '水'
              })
            </span>
          </div>
        </section>
        ...
        <section className="section animate-fade-in-up stagger-3">
          ...
          <div className="glass-card">
            <ElementChart
              elements={elements}
              distribution={analysis.elementDistribution}
            />
        ...
        <section className="section animate-fade-in-up stagger-4">
          ...
          <div className="glass-card" style={{ textAlign: 'center' }}>
            <ScoreGauge score={dailyFortune.score} />
        ...
        <section className="section animate-fade-in-up stagger-4">
          ...
          <div className="glass-card">
            ...
            <div style={{ display: 'flex', gap: 'var(--space-lg)', justifyContent: 'center', marginTop: 'var(--space-lg)' }}>
              <InfoChip
                label="사교성"
                value={analysis.socialTendency === 'extrovert' ? '외향적' : analysis.socialTendency === 'introvert' ? '내향적' : '양향적'}
              />
              <InfoChip
                label="의사결정"
                value={analysis.decisionStyle === 'intuitive' ? '직관적' : analysis.decisionStyle === 'analytical' ? '분석적' : '균형적'}
              />
            </div>
